...
if [ ! -f ${SOFTWARE_DISTRIBUTIONS}/kafka_2.11-0.10.0.0.tgz ]; then
    echo "Kafka tgz file not found. Please copy the Kafka tgz file to ${SOFTWARE_DISTRIBUTIONS} folder and retry."
    exit 1
fi
...
# add the Kafka Server
echo "Copying the Kafka Server to the ${WORKING_DIRECTORY}"
cp ${SOFTWARE_DISTRIBUTIONS}/kafka_2.11-0.10.0.0.tgz ${WORKING_DIRECTORY}
echo "Successfully copied the MySQL driver to the server pack."

echo "Copying the Kafka jars to the ${WORKING_DIRECTORY}"
cp -r ${SOFTWARE_DISTRIBUTIONS}/kafka-osgi ${WORKING_DIRECTORY}
echo "Successfully copied the Kafka jars to the server pack."
